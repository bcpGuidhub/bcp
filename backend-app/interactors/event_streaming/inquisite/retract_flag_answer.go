...
func RetractFlagAnswer(userSessionID string, conn *websocket.Conn, op int, write message.Write, messagePayload map[string]interface{}, aggregate IEventAggregate) message.IErrorMessage {
	var command commands.ICommand
	commandBus := cqrs.CQRSFacade.CommandBus()

	postDate := timestamppb.New(time.Now())
	cmdId, _ := uuid.NewUUID()
	flagPosted := &schemas.RetractFlagAnswer{
	...
	command = &commands.RetractFlagAnswer{
	...
	err = write(conn, op, &message.Message{
		Type:  message.MessageTypeRetractFlagAnswer,
		SUUID: userSessionID,
	})

...
func RetractVoteDownAnswer(userSessionID string, conn *websocket.Conn, op int, write message.Write, messagePayload map[string]interface{}, aggregate IEventAggregate) message.IErrorMessage {
	var command commands.ICommand
	commandBus := cqrs.CQRSFacade.CommandBus()

	postDate := timestamppb.New(time.Now())
	cmdId, _ := uuid.NewUUID()
	vote := &schemas.RetractVoteDownAnswer{
	...
	command = &commands.RetractVoteDownAnswer{
	...
	err = write(conn, op, &message.Message{
		Type:  message.MessageTypeRetractVoteDownAnswer,
		SUUID: userSessionID,
	})

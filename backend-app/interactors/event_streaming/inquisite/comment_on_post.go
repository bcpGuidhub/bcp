...
func CommentOnPost(userSessionID string, conn *websocket.Conn, op int, write message.Write, messagePayload map[string]interface{}, aggregate IEventAggregate) message.IErrorMessage {
	var command commands.ICommand
	commandBus := cqrs.CQRSFacade.CommandBus()

	postDate := timestamppb.New(time.Now())
	cmdId, _ := uuid.NewUUID()
	commentOnPost := &schemas.CommentOnPost{
	...
	command = &commands.CommentOnPost{
		Command: &commands.Command{
			Payload: commentOnPost,
		},
	}
	...
	err = write(conn, op, &message.Message{
		Type:  message.MessageTypeCommentOnPost,
		SUUID: userSessionID,
	})

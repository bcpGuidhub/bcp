...
func RetractAcceptAnswerToPost(userSessionID string, conn *websocket.Conn, op int, write message.Write, messagePayload map[string]interface{}, aggregate IEventAggregate) message.IErrorMessage {
	var command commands.ICommand
	commandBus := cqrs.CQRSFacade.CommandBus()

	postDate := timestamppb.New(time.Now())
	cmdId, _ := uuid.NewUUID()
	vote := &schemas.RetractAcceptAnswer{
		Id:          &schemas.UUID{Value: cmdId.String()},
		AggregateId: &schemas.UUID{Value: messagePayload["aggregate_id"].(string)},
		AnswerId:    &schemas.UUID{Value: messagePayload["answer_id"].(string)},
		Cast:        postDate,
		VoterId:     aggregate.GUUID(),
	}

	command = &commands.RetractAcceptAnswer{
	...
	err = write(conn, op, &message.Message{
		Type:  message.MessageTypeRetractAcceptAnswer,
		SUUID: userSessionID,
	})

...
func AddRevisionToAnswer(userSessionID string, conn *websocket.Conn, op int, write message.Write, messagePayload map[string]interface{}, aggregate IEventAggregate) message.IErrorMessage {
	var command commands.ICommand
	commandBus := cqrs.CQRSFacade.CommandBus()

	postDate := timestamppb.New(time.Now())
	cmdId, _ := uuid.NewUUID()
	revision := &schemas.AddRevisionToAnswer{
		Id:          &schemas.UUID{Value: cmdId.String()},
		PostDate:    postDate,
		AggregateId: &schemas.UUID{Value: messagePayload["aggregate_id"].(string)},
		AnswerId:    sanitizeTextInput(messagePayload["answer_id"].(string)),
		Author:      aggregate.GUUID(),
		Body:        sanitizeTextInput(messagePayload["body"].(string)),
		EditSummary: sanitizeTextInput(messagePayload["summary"].(string)),
	}

	command = &commands.AddRevisionToAnswer{
	...
	err = write(conn, op, &message.Message{
		Type:  message.MessageTypeAddRevisionToAnswer,
		SUUID: userSessionID,
	})

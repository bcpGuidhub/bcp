...
func VoteUpComment(userSessionID string, conn *websocket.Conn, op int, write message.Write, messagePayload map[string]interface{}, aggregate IEventAggregate) message.IErrorMessage {
	var command commands.ICommand
	commandBus := cqrs.CQRSFacade.CommandBus()

	postDate := timestamppb.New(time.Now())
	cmdId, _ := uuid.NewUUID()
	vote := &schemas.VoteUpComment{
	...
	command = &commands.VoteUpComment{
	...
	err = write(conn, op, &message.Message{
		Type:  message.MessageTypeVoteUpComment,
		SUUID: userSessionID,
	})

...
func AddAnswerToPost(userSessionID string, conn *websocket.Conn, op int, write message.Write, messagePayload map[string]interface{}, aggregate IEventAggregate) message.IErrorMessage {
	var command commands.ICommand
	commandBus := cqrs.CQRSFacade.CommandBus()

	postDate := timestamppb.New(time.Now())
	cmdId, _ := uuid.NewUUID()
	answer := &schemas.PostAnswer{
		...
		Body:        sanitizeTextInput(messagePayload["post"].(string)),
	}

	command = &commands.PostAnswer{

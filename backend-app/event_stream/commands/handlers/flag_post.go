...
const (
	// FLAG_POST_PRIVILEGE int64 = 15
	FLAG_POST_PRIVILEGE int64 = 0
)
...
// FlagPostHandler is a command handler, which handles FlagPost command and emits PostAdded.
//
// In CQRS, one command must be handled by only one handler.
// When another handler with this command is added to command processor, error will be retuerned.
type FlagPostHandler struct {
	EventBus *kafka.EventBusWriteStream
}

func (b FlagPostHandler) HandlerName() string {
	return "FlagPostHandler"
}

// NewCommand returns type of command which this handle should handle. It must be a pointer.
func (b FlagPostHandler) NewCommand() interface{} {
	return &cmdSchema.FlagPost{}
}

func (b FlagPostHandler) Handle(ctx context.Context, c interface{}) error {
	// c is always the type returned by `NewCommand`, so casting is always safe
	log.Println("---------- Running Command Handler FlagPostHandler ---------------")

	cmd := c.(*cmdSchema.FlagPost)
	...
	if reps >= FLAG_POST_PRIVILEGE {
		...
		update := bson.M{
			...
			"$addToSet": bson.M{"flags": bson.M{"flag_id": flagId,
				"details": cmd.Details,
				"reason":  cmd.Reason,

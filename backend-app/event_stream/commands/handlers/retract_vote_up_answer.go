...
const (
	RETRACT_ANSWER_UP_VOTE int64 = -10
)

// RetractVoteUpAnswerHandler is a command handler, which handles RetractVoteUpAnswer command and emits AnswerUpVoted.
//
// In CQRS, one command must be handled by only one handler.
// When another handler with this command is added to command processor, error will be retuerned.
type RetractVoteUpAnswerHandler struct {
	EventBus *kafka.EventBusWriteStream
}

func (b RetractVoteUpAnswerHandler) HandlerName() string {
	return "RetractVoteUpAnswerHandler"
}

// NewCommand returns type of command which this handle should handle. It must be a pointer.
func (b RetractVoteUpAnswerHandler) NewCommand() interface{} {
	return &cmdSchema.RetractVoteUpAnswer{}
}

func (b RetractVoteUpAnswerHandler) Handle(ctx context.Context, c interface{}) error {
	// c is always the type returned by `NewCommand`, so casting is always safe
	log.Println("---------- Running Command Handler RetractVoteUpAnswerHandler ---------------")

	cmd := c.(*cmdSchema.RetractVoteUpAnswer)
	...
	if author != cmd.VoterId {
		...
		if reps >= UP_VOTE_PRIVILEGE {
			...
			if alreadyVoted {
				...
				update := bson.M{"$pull": bson.M{"votes": bson.M{"voter_id": cmd.VoterId, "weight": ANSWER_UP_VOTE}}}

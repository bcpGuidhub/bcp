...
// FlagCommentHandler is a command handler, which handles FlagComment command and emits PostAdded.
//
...
func (b FlagCommentHandler) HandlerName() string {
	return "FlagCommentHandler"
}

// NewCommand returns type of command which this handle should handle. It must be a pointer.
func (b FlagCommentHandler) NewCommand() interface{} {
	return &cmdSchema.FlagComment{}
}

func (b FlagCommentHandler) Handle(ctx context.Context, c interface{}) error {
	// c is always the type returned by `NewCommand`, so casting is always safe
	log.Println("---------- Running Command Handler FlagCommentHandler ---------------")

	cmd := c.(*cmdSchema.FlagComment)

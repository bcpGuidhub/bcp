...
type RetractedPostDownVotedHandler struct {
	CommandBus *kafka.CommandBusWriteStream
}

func (o RetractedPostDownVotedHandler) HandlerName() string {
	return "RetractedPostDownVotedHandler"
}

func (RetractedPostDownVotedHandler) NewEvent() interface{} {
	return &evtSchema.RetractedPostDownVoted{}
}

func (o RetractedPostDownVotedHandler) Handle(ctx context.Context, e interface{}) error {
	log.Println("---------- Running EVENT Handler RetractedPostDownVotedHandler ---------------")
	event := e.(*evtSchema.RetractedPostDownVoted)
	if event == nil {
		return ErrNilEvent
	}

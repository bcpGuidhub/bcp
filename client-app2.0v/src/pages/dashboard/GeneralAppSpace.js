...
// ----------------------------------------------------------------------
export default function GeneralAppSpace() {
  const theme = useTheme();

  return (
    <Page title="Ecosystem | Guidhub">
      <Box sx={{ pt: '60px', pl: 4, pr: 4 }}>
        <Grid container>
          <Grid item xs={12}>
            <RootStyle>
              <CardContent
              ...
              >
                <Typography gutterBottom sx={{ typography: 'h3', color: '#fff' }}>
                  Bienvenue!
                </Typography>

                <Typography
                  sx={{ typography: 'h4', color: '#fff', pb: { xs: 3, xl: 5 }, maxWidth: 480, mx: 'auto', mt: 2 }}
                >
                  Démarrez votre entreprise avec Hub Tools
                </Typography>
          ...
          <Grid item xs={12} sx={{ mt: 3 }}>
            <Typography sx={{ typography: 'h6' }}>Construit pour vous par notre communauté d'experts </Typography>
          </Grid>
          {Apps.map((c) => {
            const { creator, apps } = c;
            ...
            return (
              <>
                <Grid item xs={12}>
                  <Grid container>
                    {apps.map((app) => {
                      const { items, subheader } = app;
                      return items.map((item) => (
                        <Card
                          to={item.path}
                          component={RouterLink}
                          sx={{ width: 345, boxShadow: 'none', backgroundColor: 'transparent', textDecoration: 'none' }}
                          key={item.title}
                        >
                          <CardHeader
                            sx={{
                            ...
                            avatar={
                              <Avatar
                                sx={{ bgcolor: 'rgb(98,174,87)', borderRadius: '20%', color: '#fff' }}
                                aria-label="recipe"
                              >
                                {item.icon}
                              </Avatar>

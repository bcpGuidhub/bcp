...
FinancialPlanPDF.propTypes = {
  forecast: PropTypes.array.isRequired,
  user: PropTypes.object.isRequired,
  document: PropTypes.object.isRequired
};

export default function FinancialPlanPDF({ forecast, user, document }) {
      ...
      rowPdf = row.sub_rows.map((row) => (
        <View
          ...
          style={[
            ...
            {
              backgroundColor:
                row.field === 'Besoins' || row.field === 'Ressources' ? 'rgb(255,103,1)' : row.background,
              color: row.field === 'Besoins' || row.field === 'Ressources' ? '#fff' : ''
            }
          ]}
        >
          {document?.columns?.map((column, i) => (
            <View key={Math.random()} style={[{ width: column.id === 'field' ? '30%' : width }, align(column)]}>
      ...
      <Page size="A4" style={styles.page}>
        <View style={[styles.gridContainer, styles.mb40]}>
          ...
          <View style={{ alignItems: 'right', flexDirection: 'column' }}>
            <Text style={styles.h3}>Plan de financement</Text>
            <Text>{format(new Date(), 'dd MMM yyyy')}</Text>
          </View>
        </View>

        <Text style={[styles.overline, styles.mb8]}>Plan de financement Details</Text>
            ...
            {forecast.map((row, index) => (
              <div key={Math.random()}>
                ...
                >
                  {document?.columns?.map((column, i) => (
                    <View
                      key={`${column.id}-${index}-${i}`}
                      style={[{ width: column.id === 'field' ? '30%' : width }, align(column)]}
                    >
                      <Text style={styles.subtitle2}>
                        {typeof row[column.id] === 'number' ? Math.round(row[column.id]) : row[column.id]}
                      </Text>

...
TreasuryPDF.propTypes = {
  forecast: PropTypes.array.isRequired,
  user: PropTypes.object.isRequired,
  document: PropTypes.object.isRequired
};

export default function TreasuryPDF({ forecast, user, document }) {
  ...
  const align = (column) => (column.id === 'field' ? styles.alignLeft : styles.alignRight);

  const renderRow = (row, index) => {
    let rowPdf = null;

    if (row.sub_rows.length > 0) {
      rowPdf = row.sub_rows.map((row, i) => (
        <View
          key={i}
          style={[
            styles.tableRow,
        ...
        >
          {document?.columns?.map((column, i) => (
            <View
              ...
              style={[{ width: column.id === 'field' ? '9%' : width }, align(column)]}
            >
      ...
      <Page size="A4" style={styles.page}>
        <View style={[styles.gridContainer, styles.mb40]}>
          ...
          <View style={{ alignItems: 'right', flexDirection: 'column' }}>
            <Text style={styles.h3}>Trésorerie</Text>
        ...
        {forecast.map((year, i) => (
          <View wrap={false} key={i}>
            <Text style={[styles.overline, styles.mb8]}>Trésorerie {year.year} Details</Text>
            <View style={styles.table}>
              <View style={styles.tableHeader}>
                <View style={styles.tableRow}>
                  ...
                  {document?.columns?.map((column, i) => (
                    <View key={column.label} style={[{ width }, align(column)]}>

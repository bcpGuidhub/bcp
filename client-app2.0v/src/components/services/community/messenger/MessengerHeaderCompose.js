...
MessengerHeaderCompose.propTypes = {
...
export default function MessengerHeaderCompose({
  recipientsSelectionRef,
  contacts,
  recipients,
  onAddRecipients,
  ...other
      ...
      <AutocompleteStyle>
        <Autocomplete
          ...
          renderOption={(props, recipient, { inputValue, selected }) => {
            const { user_name: name, avatar } = recipient;
          ...
          renderTags={(recipients, getTagProps) =>
            recipients.map((recipient, index) => {
              const { c_uuid: id, user_name: name, avatar } = recipient;
              return (
                <Chip
                  {...getTagProps({ index })}
                  key={id}
          ...
          renderInput={(params) => (
            <TextField {...params} placeholder="Destinataires" inputRef={recipientsSelectionRef} />
          )}
        />
      </AutocompleteStyle>
    </RootStyle>

...
MessengerRoomPopup.propTypes = {
  participant: PropTypes.object,
  isOpen: PropTypes.bool,
  onClose: PropTypes.func
};

export default function MessengerRoomPopup({ participant, isOpen, onClose, ...other }) {
  const { avatar, position, address, phone, email } = participant;
  ...
  return (
    <DialogAnimate fullWidth maxWidth="xs" open={isOpen} onClose={onClose} {...other}>
      <DialogContent sx={{ pb: 5, textAlign: 'center' }}>
        ...
        />
        <Typography variant="h6">{participant.user_name}</Typography>
